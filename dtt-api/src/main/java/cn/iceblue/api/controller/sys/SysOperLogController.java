 ...
 */
@Api(tags = "操作日志记录对象功能接口")
@RestController
@RequestMapping("common/sysoperlog")
public class SysOperLogController {
    ...
    * 分页查询
    *
    * @param sysOperLogEntity 筛选条件
    * @param pageRequest 分页对象
    * @return 查询结果
    */
    @ApiOperation("分页查询")
    @GetMapping
    public R<PageVo<SysOperLogEntity>> paginQuery(SysOperLogEntity sysOperLogEntity, PageRequest pageRequest){
        //1.分页参数
        long current = pageRequest.getPage();
        long size = pageRequest.getPageSize();
        //2.分页查询
        /*把Mybatis的分页对象做封装转换，MP的分页对象上有一些SQL敏感信息，还是通过spring的分页模型来封装数据吧*/
        Page<SysOperLogEntity> pageResult = sysOperLogService.pagingQuery(sysOperLogEntity, current,size);
        //3. 分页结果组装
    ...
    @GetMapping("/{id}")
    public R<SysOperLogEntity> queryById(@PathVariable("id") String id){
    ...
    @ApiOperation("新增数据")
    @PostMapping
    public R<Boolean> add(@RequestBody SysOperLogEntity sysOperLog){
    ...
    @ApiOperation("更新数据")
    @PutMapping
    public R<Boolean> edit(@RequestBody SysOperLogEntity sysOperLog){
